...
const char * const BITCOIN_CONF_FILENAME = "acut.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //acut-tx using stdin
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AcutCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AcutCore
    // Mac: ~/Library/Application Support/AcutCore
    // Unix: ~/.acutcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AcutCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AcutCore";
#else
    // Unix
    return pathRet / ".acutcore";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty acut.conf if it does not exist
...
//    std::string strCopyrightHolders = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS).translated, _(COPYRIGHT_HOLDERS_SUBSTITUTION));
    ...
    // Check for untranslated substitution to make sure Acut Core copyright is not removed by accident
    if (copyright_devs.find("Acut Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The Acut Core developers";
    }
