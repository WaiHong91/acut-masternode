...
static bool AppInit(int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/acut.conf are parsed in qt/bitcoin.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  acutd [options]                     Start " PACKAGE_NAME " Daemon\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see acutd -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for acutd but not for the GUI so do this here
