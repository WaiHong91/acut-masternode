...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_acut-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_acut-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_acut-qt  # macOS
void TestGUI(interfaces::Node& node)
{
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(2).indexOf(QString("URI: acut:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_acut-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
