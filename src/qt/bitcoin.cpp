...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in acut.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in acut.qrc)
...
static const char* qt_argv = "acut-qt";
...
{
    QMessageBox::critical(nullptr, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Acut Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"Acut Core - \"").translated, ArgsManager::ALLOW_ANY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(acut);
    Q_INIT_RESOURCE(acut_locale);
    ...
    /// 6. Determine availability of data directory and parse acut.conf
    ...
    // acut: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
