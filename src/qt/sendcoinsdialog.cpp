...
void SendCoinsDialog::send(QList<SendCoinsRecipient> recipients)
{
    ...
    if (m_coin_control->IsUsingCoinJoin()) {
        ...
        if (nInputs >= 10 && m_coin_control->IsUsingCoinJoin()) {
            ...
            questionString.append("<a style='" + GUIUtil::getThemedStyleQString(GUIUtil::ThemedStyle::TS_COMMAND) + "' href=\"https://docs.acut.org/en/stable/wallets/acutcore/coinjoin-instantsend.html#inputs\">");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Acut address"));
        }
        else // Valid address
        {
