...
"""Class for acutd node under test"""
...
class TestNode():
    """A class for representing a acutd node under test.
    ...
    def __init__(self, i, datadir, extra_args_from_options, *, chain, rpchost, timewait, timeout_factor, bitcoind, bitcoin_cli, mocktime, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False):
        ...
        self.chain = chain
        self.bitcoinconf = os.path.join(self.datadir, "acut.conf")
    ...
    def __del__(self):
        # Ensure that we don't leave any acutd processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time acutd is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by acutd, and
        ...
        self.running = True
        self.log.debug("acutd started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the acutd process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'acutd exited with status {} during initialization'.format(self.process.returncode)))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. acutd still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to acutd after {}s".format(self.rpc_timeout))
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to acutd
        expected_msg: regex that stderr should match when acutd fails

        Will throw if acutd starts without an error.
        Will throw if an expected_msg is provided and it does not match acutd's stdout."""
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug('acutd failed to start: %s', e)
                self.running = False
                self.process = None
                ...
                if expected_msg is None:
                    assert_msg = "acutd should have exited with an error"
                else:
                    assert_msg = "acutd should have exited with expected error " + expected_msg
...
class TestNodeCLI():
    """Interface to acut-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.acutcli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with acut-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run acut-cli command. Deserializes returned string as python object."""
        pos_args = [arg_to_cli(arg) for arg in args]
        named_args = [str(key) + "=" + arg_to_cli(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same acut-cli call"
        ...
        self.log.debug("Running acut-cli command: %s" % command)
