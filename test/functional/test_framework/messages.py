...
# Objects that map to acutd objects, which can be serialized/deserialized
...
class CBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(acuthash(r))
            self.hash = encode(acuthash(r)[::-1], 'hex_codec').decode('ascii')
...
class CompressibleBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(acuthash(r))
            self.hash = int(encode(acuthash(r)[::-1], 'hex_codec'), 16)
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in acutd indicates these should be deserialized as blocks
