...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave acutds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop acutds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use acut-cli instead of RPC for all commands")
        parser.add_argument("--acutd-arg", dest="acutd_extra_args", default=[], action="append",
                            help="Pass extra args to all acutd instances")
    ...
    def setup(self):
        ...
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/acutd' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/acut-cli' + config["environment"]["EXEEXT"])

        self.extra_args_from_options = self.options.acutd_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: acutds were not stopped and may still be running")
    ...
    def dynamically_initialize_datadir(self, chain, node_p2p_port, node_rpc_port):
        ...
        if chain == 'testnet3':
            ...
            chain_name_conf_arg_value = '1'
        with open(os.path.join(data_dir, "acut.conf"), 'w', encoding='utf8') as f:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as acutd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in acut.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if acutd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("acutd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if acut-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("acut-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if acut-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("acut-cli has not been compiled.")

    def is_cli_compiled(self):
        """Checks whether acut-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether acut-wallet was compiled."""
...
class AcutTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_acut_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False, hpmn_count=0):
        ...
        self.set_acut_dip8_activation(200)

        # LLMQ default test params (no need to pass -llmqtestparams)
        self.llmq_size = 3
        self.llmq_threshold = 2

        # This is nRequestTimeout in acut-q-recovery thread
    ...
    def set_acut_dip8_activation(self, activate_after_block):
    ...
    def set_acut_llmq_test_params(self, llmq_size, llmq_threshold):
    ...
    def remove_masternode(self, idx):
        mn = self.mninfo[idx]
        rawtx = self.nodes[0].createrawtransaction([{"txid": mn.collateral_txid, "vout": mn.collateral_vout}], {self.nodes[0].getnewaddress(): 999.8844})
