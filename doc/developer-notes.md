...
- [Development guidelines](#development-guidelines)
    - [General Acut Core](#general-acut-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Acut Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Acut Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/acutpay/acut/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_acut
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_acut --log_level=test_suite
$ valgrind -v --leak-check=full src/acutd -printtoconsole
...
# A coverage report will now be accessible at `./test_acut.coverage/index.html`.
```
...
To profile a running acutd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep acutd` -- sleep 60
```
...
Acut Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 8844.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 8855 for connections and services them.
...
However, in open source software such as Acut Core, where everyone uses
...
pay attention to for reviewers of Acut Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Acut Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against Acut Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `acut-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `acut-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Acut Core, the wallet was always
