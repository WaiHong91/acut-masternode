...
*Setup instructions for a Gitian build of Acut Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Acut
...
- [Building Acut Core](#building-acut-core)
...
Clone the git repositories for Acut Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/acutpay/acut
...
in the Acut Core repository under 'Fetch and create inputs' to install sources which require
...
To build Acut Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Acut Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/acut/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/acutpay/acut
...
For example:
```bash
URL=https://github.com/crowning-/acut.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit acut=${COMMIT} --url acut=${URL} ../acut/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit acut=${COMMIT} --url acut=${URL} ../acut/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit acut=${COMMIT} --url acut=${URL} ../acut/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the acut git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../acut/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/acut
SIGPATH=/some/root/path/acut-detached-sigs

./bin/gbuild --url acut=${BTCPATH},signature=${SIGPATH} ../acut/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/acut-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/acut-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/acut-osx-build.assert
```
...
[acut/gitian.sigs](https://github.com/acutpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
