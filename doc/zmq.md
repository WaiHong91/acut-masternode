...
The Acut Core daemon can be configured to act as a trusted "border
router", implementing the acut wire protocol and relay, making
...
The ZeroMQ feature in Acut Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building acutd:
...
For instance:
    ...
    $ acutd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/acutd.tx.raw \
...
These options can also be provided in acut.conf.
...
From the perspective of acutd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into acutd directly. Furthermore, no information is
...
using. Acutd appends an up-counting sequence number to each
